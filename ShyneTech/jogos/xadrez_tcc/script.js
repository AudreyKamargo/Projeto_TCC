function inicia_jogo(){
    ...
    game.on("check", () => {
        alert("O rei está em xeque!");
      });
      
      // Roda o jogo
      while (!game.isFinished()) {
        // Obtém o movimento do jogador atual
      ...
      // Importa o módulo do jogo de xadrez
const chess = require("chessjs");

// Inicializa o jogo
const game = chess.Board();

// Define uma função para verificar se o rei está em xeque
function isInCheck() {
  // Obtém a posição do rei
  const kingPos = game.findPiece({ type: chess.PieceType.KING });

  // Verifica se qualquer peça do oponente pode capturar o rei
  for (const piece of game.pieces({ color: game.turn().opposite() })) {
    if (piece.moves().some((move) => move.to === kingPos && isAttacking(piece, kingPos))) {
      return true;
    }
  }
...
// Define uma função para verificar se uma peça está atacando uma posição
function isAttacking(piece, pos) {
  // Verifica se a peça pode se mover para a posição
  return piece.moves().some((move) => move.to === pos);
}

// Define um evento para emitir um alerta quando o rei estiver em xeque
game.on("check", () => {
  // Emite um alerta diferente dependendo da gravidade da ameaça ao rei
  const threat = getThreat(kingPos);
  switch (threat) {
    case "simple":
      alert("O rei está em xeque!");
      break;
    case "double":
      alert("O rei está em xeque duplo!");
      break;
    case "mate":
      alert("O rei está em xeque mate!");
      break;
  }
});
...
// Define uma função para obter a ameaça ao rei
function getThreat(pos) {
  // Obtém todas as peças que estão atacando a posição do rei
  const attackers = game.pieces({
    color: game.turn().opposite(),
    moves: (piece) => piece.moves().some((move) => move.to === pos),
  });

  // Verifica se há duas ou mais peças atacando a posição do rei
  if (attackers.length >= 2) {
    return "double";
  }

  // Verifica se a posição do rei está sob ataque de um peão em promoção
  for (const piece of game.pieces({ color: game.turn().opposite(), type: chess.PieceType.PAWN })) {
    if (piece.moves().some((move) => move.to === pos && move.promotion === true)) {
      return "double";
    }
  }

  // Se não houver nenhuma ameaça grave, a ameaça é considerada simples
  return "simple";
}
